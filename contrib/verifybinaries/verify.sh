...
###   topazcore.org and topaz.org and compares them.
...
WORKINGDIR="/tmp/topaz_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://topazcore.org"
HOST2="https://topaz.org"
BASEDIR="/bin/"
VERSIONPREFIX="topaz-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'topaz-',
...
if [ $? -ne 0 ]; then
   echo "topaz.org failed to provide signature file, but topazcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "topaz.org and topazcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the TOPAZ Core binary release signing key installed?"
   fi
