...
const char * const TOPAZ_CONF_FILENAME = "topaz.conf";
const char * const TOPAZ_PID_FILENAME = "topazd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "topazd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TOPAZ
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TOPAZ
    // Mac: ~/Library/Application Support/TOPAZ
    // Unix: ~/.topaz
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TOPAZ";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TOPAZ";
#else
    // Unix
    return pathRet / ".topaz";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", TOPAZ_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", TOPAZ_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        ...
        strYear.replace(strYear.find("2019"), sizeof("2019")-1, "2009-2019");
