...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " topaz-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  topaz-tx [options] <hex-tx> [commands]  Update hex-encoded topaz transaction\n" +
            "or:     topaz-tx [options] -create [commands]   Create hex-encoded topaz transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded topaz transaction
